# 更新日志
- 26.09.10 v2.5.3 需要重启
  1. 优化默认关键词与网址处理，补充一次性密码等格式；保留自定义规则优先级。
  2. 汇总 API 102 系统作用域、初始化回退、模块日志及 Release 混淆修复。
  3. 统一 APK 与发布版本，发布前执行测试和签名校验。澎湃 OS 4 仍待真机验证。
  4. [详细变更与验证范围](docs/release-2.5.3.md)
